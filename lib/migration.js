...
class Migration {
  ...
  static create(name, directory) {
    ...
    // file name looks like migrations/1391877300255_migration-title/index.js
    const migration = new Date().getTime() + '_' + name
    mkdirp.sync(directory + '/' + migration + '/sql')

    const new_file = util.format('%s/%s/index.js', directory, migration);
    ...
    // Create SQL files
    fs.openSync(directory + '/' + migration + '/sql' + '/up.sql', 'w');
    fs.openSync(directory + '/' + migration + '/sql' + '/down.sql', 'w');
  ...
  constructor(db, migrationPath, { up, down } = {}, options = {}, log = console.log) {
    ...
    this.name = migrationPath.split('/').slice(-2)[0];
    this.up = up;
    this.down = down;
